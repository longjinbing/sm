...
import com.ljb.Exception.AccountException;
...
public class UserRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        try {sysUser=(SysUser)session.getAttribute("curUser");
        	...
        	sysUser=sysUserService.queryByName(username);
        	session.setAttribute("curUser", sysUser);
        }
        List<String> permissions=sysMenuService.permissions(sysUser.getId());
        List<String> roles=sysRoleService.roleNames(sysUser.getId());
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
		...
		SysUser sysUser=sysUserService.queryByNamePass(username,password);
		if(sysUser==null) {
			return null;
		}
